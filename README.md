...
или
sudo grubby --set-default /boot/config-5.10.11
...
Перегрузаемся, заходим vagrant ssh смотрим версию gcc --version (7.4.0 должна быть)
...
cp /boot/config-5.10.11....elrepo   /usr/src/kernel/linux-5.10.11/.config
cp /usr/src/kernel/linux-5.10.11/.config /usr/src/kernel/linux-5.10.11/.config.oldconfig

ядро  kernel-ml 5.10.11 можно удалить 

Зайти в распакованную папку linux-5.10.11

make oldconfig - ответить на вопрос о новых значениях в конфиге
...
Далее стандартная сборка ядра:

make -j3 <-- где j3 количество ядер на компе, чем больше задействовать тем быстрее будет собираться
make modules_install - инсталяция модулей ядра
make install - инсталяция нового ядра

Обновляем конфигурацию загрузчика как в первой части установки ядра в полуавтоматическом режиме.
